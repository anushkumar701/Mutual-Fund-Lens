...
const ComparedFundCard = memo(function ComparedFundCard({ fund, color, onRemove }) {
  ...
  return (
    <div className="card p-5 relative border-l-4" style={{ borderLeftColor: color }}>
      ...
      <div className="space-y-3 pr-8">
          ...
          {(() => {
            ...
            return (
              <div className="col-span-2 pt-2 border-t border-slate-100 dark:border-slate-700/50 flex justify-between">
                <div>
                  <span className="text-slate-500 flex items-center gap-1">Min SIP
                    <span className="relative group cursor-help text-[9px] bg-slate-200 dark:bg-slate-700 rounded-full w-3 h-3 flex items-center justify-center font-bold" aria-label="Estimated minimum SIP amount">
                      ?
                      <span className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-32 p-1.5 bg-slate-800 text-white text-[10px] rounded shadow-lg z-50 text-center font-normal normal-case leading-tight" role="tooltip">
                ...
                <div>
                  <span className="text-slate-500 flex items-center gap-1">Min Lumpsum
                    <span className="relative group cursor-help text-[9px] bg-slate-200 dark:bg-slate-700 rounded-full w-3 h-3 flex items-center justify-center font-bold" aria-label="Estimated minimum lumpsum amount">
                      ?
                      <span className="hidden group-hover:block absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-36 p-1.5 bg-slate-800 text-white text-[10px] rounded shadow-lg z-50 text-center font-normal normal-case leading-tight" role="tooltip">
        ...
        {hl && latestNav && (
          <div className="mt-2" aria-label={`52-week range: low ₹${hl.low.toFixed(2)} to high ₹${hl.high.toFixed(2)}`}>
