...
function NavBadge({ storageKey, color }) {
  ...
  return (
    <span className={`absolute -top-0.5 -right-0.5 ${color} text-white text-[9px] font-bold rounded-full w-4 h-4 flex items-center justify-center leading-none shadow-sm`}>
      {list.length}
    </span>
  );
}
    ...
    <>
      ...
      <nav className="hidden md:flex fixed top-0 left-0 right-0 z-50 h-16 bg-white dark:bg-[#0d1117] border-b border-slate-200 dark:border-slate-800 shadow-sm items-center px-6 gap-6">
        ...
        <div className="flex items-center gap-2 text-xs">
          {watchlist.length > 0 && (
            <span className="flex items-center gap-1 bg-amber-50 dark:bg-amber-950 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 rounded-full px-2.5 py-1 font-medium">
              ⭐ {watchlist.length} saved
            </span>
          )}
          {compareList.length > 0 && (
            <span className="flex items-center gap-1 bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700 rounded-full px-2.5 py-1 font-medium">
