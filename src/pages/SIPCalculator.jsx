...
const TAX_RULES = {
  ...
  2022: {
    equity: { stcgRate: 0.15, ltcgRate: 0.10, ltcgThresholdMonths: 12, ltcgExemption: 100000 },
    debt:   { allRate: 'slab', ltcgMonths: 36, ltcgRate: 0.20, indexation: true },
  ...
  2024: {
    equity: { stcgRate: 0.20, ltcgRate: 0.125, ltcgThresholdMonths: 12, ltcgExemption: 125000 },
    debt:   { allRate: 'slab' },
...
function resolveRules(sellFY) {
  const keys = Object.keys(TAX_RULES).map(Number).sort((a, b) => a - b);
  const applicable = keys.filter(k => k <= sellFY);
  const key = applicable.length > 0 ? applicable[applicable.length - 1] : keys[keys.length - 1];
  return { rules: TAX_RULES[key], fyKey: key, fyLabel: `FY ${key}-${String(key + 1).slice(-2)}` };
}
...
function buildTaxResult({ fundType, gain, holdingMonths, taxSlab, sellDate }) {
  ...
  if (gain <= 0) {
    return {
      ...
      steps: [
        { icon: '📉', title: 'You made a loss', body: `You sold for ${fmt(Math.abs(gain))} less than you bought. No tax is due.` },
        { icon: '🔄', title: 'Carry-forward benefit', body: 'This loss can be carried forward for up to 8 financial years and set off against future capital gains.' },
        { icon: '💡', title: 'Tip', body: 'Short-term capital loss can offset both short-term and long-term gains. Long-term loss can only offset long-term gains.' },
      ],
  ...
  if (isEquity) {
    if (holdingMonths < rules.equity.ltcgThresholdMonths) {
      ...
      return {
        ...
        rulesNote: rules.note,
        steps: [
          { icon: '⏱️', title: `Held only ${holdMo} months`, body: `Less than 12 months = Short-Term. The government treats this like a quick profit and taxes it more heavily.` },
          { icon: `💸`, title: `STCG rate is ${ratePct}% flat`, body: `Your profit of ${fmt(gain)} is fully taxable. Tax = ${fmt(gain)} × ${ratePct}% = ${fmt(tax)}.` },
          { icon: '💡', title: 'Money-saving tip', body: `If you wait until this fund completes 12 months, the STCG rate of ${ratePct}% drops to LTCG rate of ${(rules.equity.ltcgRate * 100).toFixed(1)}% — and first ${fmt(rules.equity.ltcgExemption)} is tax-free!` },
        ],
        pillColor: 'bg-orange-100 dark:bg-orange-900/40 text-orange-800 dark:text-orange-300',
      };
    } else {
      ...
      return {
        ...
        rulesNote: rules.note,
        steps: [
          { icon: '🏆', title: `Held ${holdYrs} years — LTCG applies`, body: `You held for more than 12 months. This qualifies as Long-Term Capital Gain (LTCG), which is taxed at a lower rate.` },
          { icon: '🎁', title: `First ${fmt(exemption)} is tax-free`, body: `Every financial year, the government gives a free exemption of ${fmt(exemption)} on equity LTCG. You don't pay even 1 rupee on this portion.` },
          { icon: '🧮', title: 'How tax is calculated', body: taxableGain <= 0 ? `Your gain of ${fmt(gain)} is within the ${fmt(exemption)} exemption limit — tax payable is ₹0!` : `Taxable gain = ${fmt(gain)} − ${fmt(exemption)} = ${fmt(taxableGain)}. Tax = ${fmt(taxableGain)} × ${ratePct}% = ${fmt(tax)}.` },
        ],
        pillColor: 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300',
      };
    }
  }
  ...
  if (useSlabRate) {
    ...
    return {
      ...
      rulesNote: rules.note,
      steps: [
        { icon: '⚖️', title: 'Debt funds lost LTCG benefit (Apr 2023)', body: `From April 1, 2023, all debt fund gains — whether you held for 1 month or 10 years — are added to your income and taxed at your income tax slab rate.` },
        { icon: '🏦', title: `Your slab rate is ${taxSlab}%`, body: `Tax = ${fmt(gain)} × ${taxSlab}% = ${fmt(tax)}. This is the same as how Fixed Deposit interest is taxed.` },
        { icon: '💡', title: 'Smarter alternatives', body: `If you want lower tax, consider Equity Savings Funds (65%+ equity) which qualify for equity LTCG, or Tax-Free Bonds for post-tax efficiency.` },
      ],
  ...
  const tax = holdingMonths >= (rules.debt.ltcgMonths || 36)
    ? gain * (rules.debt.ltcgRate || 0.20)
    : gain * (taxSlab / 100);
  return {
    taxType: holdingMonths >= (rules.debt.ltcgMonths || 36) ? `LTCG @ ${(rules.debt.ltcgRate * 100).toFixed(0)}% (with indexation)` : `STCG @ slab ${taxSlab}%`,
    taxAmount: tax,
    fyLabel,
    rulesNote: rules.note,
    steps: [
      { icon: '📅', title: 'Old debt rules applied', body: `Your sell date falls under older tax rules (${fyLabel}). LTCG on debt was ${(rules.debt.ltcgRate * 100).toFixed(0)}% with indexation for holds > ${rules.debt.ltcgMonths} months.` },
    ],
    pillColor: 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300',
  };
}
...
export default function SIPCalculator() {
  ...
  const dateFundSearch = useMemo(() => {
    ...
    return (funds || []).filter(f => f.schemeName.toLowerCase().includes(q) || String(f.schemeCode).includes(q)).slice(0, 8);
  ...
  const dateOptimizerResults = useMemo(() => {
    ...
    for (let d = 1; d <= 28; d++) {
      ...
      if (res) results.push({ date: d, xirr: res.xirr, currentValue: res.currentValue, invested: res.invested });
    }
    ...
    const navArr = [...navData].reverse().map(d => parseFloat(d.nav)).filter(v => v > 0);
    ...
    for (let day = 1; day <= 28; day++) {
      ...
      for (let m = 0; m < monthsBack; m++) {
        ...
        const idx = Math.min(navArr.length - 1, Math.round(m * 21 + (day / 28) * 5));
        const nav = navArr[navArr.length - 1 - idx];
        if (nav && nav > 0) { totalInvested += dateSipAmount; totalUnits += dateSipAmount / nav; }
      }
      ...
      let xirr = null;
      try {
        let lo = -0.5, hi = 2.0, rate = 0;
        for (let iter = 0; iter < 100; iter++) {
          rate = (lo + hi) / 2;
          if (Math.abs(hi - lo) < 1e-8) break;
          if (Math.abs(rate) < 1e-10) { rate = 0; break; }
          const fv = dateSipAmount * (1 + rate) * (Math.pow(1 + rate, n) - 1) / rate;
          if (fv > currentValue) hi = rate; else lo = rate;
        }
  ...
  const bestDateResult  = dateOptimizerResults.length > 0 ? [...dateOptimizerResults].sort((a,b) => (b.xirr??-99)-(a.xirr??-99))[0] : null;
  const worstDateResult = dateOptimizerResults.length > 0 ? [...dateOptimizerResults].sort((a,b) => (a.xirr??99)-(b.xirr??99))[0] : null;
    ...
    <div className="min-h-screen pb-24 md:pb-8 md:pt-20 pt-16">
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="flex bg-slate-100 dark:bg-slate-800 rounded-xl p-1 gap-1 overflow-x-auto no-scrollbar">
          {[['calc','📈 SIP / Lumpsum'],['swp','💸 SWP'],['goal','🎯 Goal'],['elss','🧾 ELSS Tax'],['fire','🔥 FIRE'],['tax','🏛️ Tax P&L'],['date','📅 SIP Date']].map(([id, label]) => (
        ...
        <div className="grid lg:grid-cols-[420px,1fr] gap-6">
          ...
          <div className="space-y-5">
            ...
            <div className="card p-4 flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-700 rounded-lg p-0.5 gap-0.5">
                  <button id="mode-sip" onClick={() => setIsLumpsum(false)}
                  ...
                  <button id="mode-lumpsum" onClick={() => setIsLumpsum(true)}
                    className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all ${isLumpsum ? 'bg-white dark:bg-slate-600 text-blue-600 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
                    Lumpsum
                  </button>
                </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button id="inflation-toggle" onClick={() => setInflationMode(!inflationMode)}
                  className={`relative w-11 h-6 rounded-full transition-colors ${inflationMode ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
                  <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${inflationMode ? 'translate-x-5' : 'translate-x-0'}`} />
                </button>
              </div>
            </div>
            ...
            <div className="card p-5 space-y-6">
              ...
              <SIPSlider id="years-slider" label="Investment Duration" value={years} onChange={setYears} min={1} max={30} suffix=" yr" />
              <SIPSlider id="return-slider" label="Expected Annual Return" value={returns} onChange={setReturns} min={1} max={30} step={0.5} suffix="%" />
              <SIPSlider id="expense-ratio-slider" label="Expense Ratio" value={expenseRatio} onChange={setExpenseRatio} min={0} max={3} step={0.05} suffix="%" />
              {expenseRatio > 0 && (
                <div className={`flex items-center justify-between text-xs rounded-lg px-3 py-2 ${effectiveReturnWarning ? 'bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800' : 'bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800'}`}>
              ...
              {!isLumpsum && (
                <SIPSlider id="stepup-slider" label="Annual Step-Up" value={stepUp} onChange={setStepUp} min={0} max={25} suffix="%" />
              )}
              {inflationMode && (
                <SIPSlider id="inflation-slider" label="Inflation Rate" value={inflation} onChange={setInflation} min={1} max={10} suffix="%" />
              )}
            </div>
          </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              ...
              <ResultCard label="Maturity Value" value={formatINR(result.maturity)} accent sub={`${wealthMultiple}× wealth multiple`} />
              {inflationMode && realValue !== null && (
                <ResultCard label={`Real Value (at ${inflation}% inflation)`} value={formatINR(realValue)} sub="Inflation-adjusted" />
              )}
            </div>
            ...
            <div className="card p-4 space-y-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/50 dark:to-teal-950/50 border-emerald-100 dark:border-emerald-800">
              <div>
                ...
                <div className="flex items-center gap-3">
                  <div className="text-3xl font-black text-emerald-600 dark:text-emerald-400 tabular-nums">{wealthMultiple}×</div>
                  <p className="text-xs text-slate-600 dark:text-slate-300">Your money grew {wealthMultiple} times in {years} year{years > 1 ? 's' : ''}.</p>
                </div>
              </div>
            ...
            <div className="card overflow-hidden">
              <button onClick={() => setShowTable(!showTable)} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
            ...
            <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
                  <strong>Disclaimer:</strong> Estimated returns are based on assumed rates and are not guaranteed. Past performance does not indicate future results. Please consult a SEBI-registered financial advisor before investing.
                </p>
              </div>
            </div>
          </div>
        ...
        {pageTab === 'swp' && (
          <div className="grid lg:grid-cols-[420px,1fr] gap-6">
            <div className="card p-6 space-y-6">
              ...
              <SIPSlider id="swp-investment" label="Total Investment (Initial Corpus)" value={swpInvestment} onChange={setSwpInvestment} min={50000} max={100000000} step={50000} prefix="₹" formatFn={(v) => formatINR(v)} />
              <div className="flex flex-wrap gap-2">
                {[500000, 1000000, 5000000, 10000000].map((preset) => (
                  <button key={preset} onClick={() => setSwpInvestment(preset)}
              ...
              <SIPSlider id="swp-withdrawal" label="Monthly Withdrawal Amount" value={swpWithdrawal} onChange={setSwpWithdrawal} min={1000} max={1000000} step={1000} prefix="₹" formatFn={(v) => formatINR(v)} />
              ...
              <SIPSlider id="swp-return" label="Expected Annual Return" value={swpReturn} onChange={setSwpReturn} min={1} max={30} step={0.5} suffix="%" />
              <SIPSlider id="swp-years" label="Withdrawal Duration" value={swpYears} onChange={setSwpYears} min={1} max={40} suffix=" yr" />
            </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-3">
                ...
                <ResultCard label="Final Value (Corpus Remaining)" value={formatINR(swpResult.finalValue)} accent={swpResult.finalValue > 0} sub={swpResult.finalValue === 0 ? '🚫 Corpus exhausted early' : '💼 Portfolio left'} />
                <ResultCard label="Total Returns Earned" value={formatINR(swpResult.totalReturns)} sub="Accrued interest" />
              </div>
                ...
                <div className="card p-5 border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/40 text-red-800 dark:text-red-300">
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-1">At your current withdrawal rate, your corpus will last only <strong className="text-red-700 dark:text-red-400">{swpResult.ranOutYear} Year{swpResult.ranOutYear > 1 ? 's' : ''}{swpResult.ranOutMonth > 0 ? ` & ${swpResult.ranOutMonth} Month${swpResult.ranOutMonth > 1 ? 's' : ''}` : ''}</strong>.</p>
                      <p className="text-xs mt-2 text-slate-500">💡 Try lowering monthly withdrawal or a higher return allocation.</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="card p-5 border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300">
                  <div className="flex gap-3 items-start">
                    ...
                    <div>
                      ...
                      <p className="text-xs mt-1">Your corpus lasts the full {swpYears} years and still grows to <strong>{formatINR(swpResult.finalValue)}</strong>.</p>
                      <p className="text-xs mt-1">Total withdrawn: <strong>{formatINR(swpResult.totalWithdrawn)}</strong>.</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="card overflow-hidden">
                <button onClick={() => setSwpShowTable(!swpShowTable)} className="w-full flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
        ...
        {pageTab === 'goal' && (
          <div className="grid lg:grid-cols-[420px,1fr] gap-6">
            <div className="card p-6 space-y-6">
              ...
              <SIPSlider id="goal-target" label="Target Amount" value={goalTarget} onChange={setGoalTarget} min={100000} max={100000000} step={100000} prefix="₹" formatFn={(v) => formatINR(v)} />
              <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="card p-4 text-center"><p className="text-xs text-slate-500 mb-1">Total Invested</p><p className="font-bold text-slate-900 dark:text-white">{formatINR(goalTotal)}</p></div>
                <div className="card p-4 text-center"><p className="text-xs text-slate-500 mb-1">Gains</p><p className="font-bold text-emerald-600 dark:text-emerald-400">{formatINR(Math.max(0, goalTarget - goalTotal))}</p></div>
                <div className="card p-4 text-center"><p className="text-xs text-slate-400 mb-1">Goal</p><p className="font-bold text-slate-900 dark:text-white">{formatINR(goalTarget)}</p></div>
              </div>
              <div className="card p-4 bg-emerald-50 dark:bg-emerald-950 border-emerald-200 dark:border-emerald-800">
                <p className="text-xs text-emerald-700 dark:text-emerald-300 leading-relaxed">💡 <strong>Tip:</strong> Even a 10% yearly step-up can significantly reduce the required monthly SIP amount.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {pageTab === 'elss' && (
          <div className="grid lg:grid-cols-[420px,1fr] gap-6">
            <div className="card p-6 space-y-6">
              ...
              <SIPSlider id="elss-amount" label="Annual ELSS Investment" value={elssAmount} onChange={setElssAmount} min={500} max={150000} step={500} prefix="₹" formatFn={(v) => formatINR(v)} />
              <div>
                ...
                <div className="flex gap-2 flex-wrap" role="radiogroup" aria-labelledby="tax-slab-label">
                  {[5,10,15,20,25,30].map(slab => (
                    <button key={slab} onClick={() => setTaxSlab(slab)} role="radio" aria-checked={taxSlab===slab}
            ...
            <div className="space-y-4">
              <div className="card p-4 bg-amber-50 dark:bg-amber-950 border border-amber-300 dark:border-amber-700">
                <p className="text-xs text-amber-800 dark:text-amber-200 font-semibold">
                  ⚠️ Old Tax Regime Only — Section 80C deductions (including ELSS) are <strong>not available under the New Tax Regime</strong> (default from FY 2023-24). If you have opted for the new regime, your tax saving from ELSS is ₹0.
                  <a href="https://incometaxindia.gov.in/" target="_blank" rel="noopener noreferrer" className="underline ml-1 text-amber-700 dark:text-amber-300" aria-label="Income Tax India website for regime information">Learn more →</a>
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="card p-5 text-center"><p className="text-xs text-slate-400 mb-1">80C Eligible</p><p className="text-2xl font-bold text-slate-900 dark:text-white">{formatINR(elssResult.eligible)}</p></div>
                <div className="card p-5 text-center"><p className="text-xs text-slate-400 mb-1">Effective Cost</p><p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{formatINR(elssResult.effectiveCost)}</p></div>
              </div>
              <div className="card p-5 space-y-3">
                ...
                <div className="grid sm:grid-cols-2 gap-3 text-xs text-slate-600 dark:text-slate-400">
                  {[['⏱️','Shortest Lock-in','Only 3 years — vs 5 years for PPF/NSC'],['📈','Equity Returns','Historically 12–15% CAGR over 10+ years'],['💸','Tax Efficient','LTCG up to ₹1L is tax-free annually'],['🔄','SIP Friendly','Start with ₹500/month in ELSS funds']].map(([icon, title, desc]) => (
        ...
        {pageTab === 'fire' && (() => {
          ...
          const calculateMilestoneAge = (targetPct) => {
            ...
            if (fireCurrentCorpus >= milestoneTarget) return { reached: true, age: fireCurrentAge, years: 0 };
            const S = monthlySIP;
            const rateVal = fireReturnRate / 100 / 12;
            if (rateVal <= 0) {
              const totalSaved = fireCurrentCorpus + S * yearsToFire * 12;
              if (S <= 0 || totalSaved < milestoneTarget) return { reached: false, age: null, years: null };
              const months = S > 0 ? (milestoneTarget - fireCurrentCorpus) / S : Infinity;
              const yrs = months / 12;
            ...
            if (denominator <= 0 || numerator / denominator <= 0) return { reached: false, age: null, years: null };
            const months = Math.log(numerator / denominator) / Math.log(1 + rateVal);
            if (!isFinite(months) || months < 0) return { reached: false, age: null, years: null };
            const yrs = months / 12;
            return { reached: false, age: Math.round(fireCurrentAge + yrs), years: parseFloat(yrs.toFixed(1)) };
          };
            ...
            <div className="space-y-6">
              ...
              <div className="card p-5 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-950/40 dark:to-amber-950/40 border-orange-100 dark:border-orange-900/50">
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                  FIRE is the state where your accumulated investments yield returns that fully cover your cost of living. Under the classic <strong className="text-orange-700 dark:text-orange-400">4% Rule</strong>, saving 25 times your annual expenses allows you to live off returns indefinitely.
                </p>
              </div>
              ...
              <div className="grid lg:grid-cols-[400px,1fr] gap-6">
                ...
                <div className="card p-5 space-y-6 border-slate-100 dark:border-slate-800">
                  ...
                  <SIPSlider accent="orange" id="fire-monthly-expense" label="Current Monthly Expenses" value={fireMonthlyExpense} onChange={setFireMonthlyExpense} min={5000} max={1000000} step={1000} prefix="₹" formatFn={(v) => formatINR(v)} />
                  <div className="grid grid-cols-2 gap-4">
                    <SIPSlider accent="orange" id="fire-current-age" label="Current Age" value={fireCurrentAge} onChange={setFireCurrentAge} min={15} max={75} suffix=" yrs" />
                    <SIPSlider accent="orange" id="fire-retire-age" label="Target Retire Age" value={fireRetireAge} onChange={setFireRetireAge} min={20} max={85} suffix=" yrs" />
                  </div>
                  <SIPSlider accent="orange" id="fire-corpus" label="Current Saved Corpus" value={fireCurrentCorpus} onChange={setFireCurrentCorpus} min={0} max={100000000} step={10000} prefix="₹" formatFn={(v) => formatINR(v)} />
                  <SIPSlider accent="orange" id="fire-return-rate" label="Expected Annual Return" value={fireReturnRate} onChange={setFireReturnRate} min={4} max={25} step={0.5} suffix="%" />
                  <div className="grid grid-cols-2 gap-4">
                    <SIPSlider accent="orange" id="fire-withdrawal-rate" label="Safe Withdrawal Rate" value={fireWithdrawalRate} onChange={setFireWithdrawalRate} min={1} max={10} step={0.1} suffix="%" />
                    <SIPSlider accent="orange" id="fire-inflation" label="Expected Inflation" value={fireInflation} onChange={setFireInflation} min={1} max={15} step={0.5} suffix="%" />
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div className="card p-6 bg-gradient-to-br from-orange-500 to-red-600 text-white border-none shadow-lg relative overflow-hidden">
                    ...
                    <p className="text-xs text-orange-50 font-medium mt-2 leading-relaxed">
                      Inflation-adjusted corpus required to sustain {fmt(futureMonthlyExpense)}/month at age {fireRetireAge} under a {fireWithdrawalRate}% safe withdrawal rate.
                    </p>
                  </div>
                  ...
                  <div className="card p-5 border-slate-100 dark:border-slate-800">
                    <h4 className="font-bold text-sm text-slate-800 dark:text-white mb-4 flex items-center gap-1.5"><span>🗓️</span> Retirement Milestones</h4>
                    <div className="space-y-4">
                      {[0.25, 0.5, 0.75, 1.0].map((pct) => {
                        ...
                        return (
                          <div key={pct} className="relative flex gap-4 items-start pb-4 last:pb-0">
                            {pct !== 1.0 && <div className="absolute left-3 top-6 bottom-0 w-0.5 bg-slate-100 dark:bg-slate-800" />}
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black z-10 shadow-sm ${isCompleted ? 'bg-emerald-500 text-white' : 'bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border-2 border-orange-200 dark:border-orange-800'}`}>
                              {isCompleted ? '✓' : `${pct * 100}%`}
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-center gap-4">
                                ...
                                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${isCompleted ? 'bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400' : 'bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-400'}`}>
                                  {isCompleted ? 'Achieved!' : est.age ? `Est. Age ${est.age}` : '—'}
                                </span>
                              </div>
                              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                                {isCompleted ? 'Milestone cleared!' : est.years !== null ? `Reachable in ${est.years} years of regular SIP investing` : 'Need active monthly investment plan'}
                              </p>
                            </div>
                          </div>
                        );
                  ...
                  <div className="card p-5 border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/10">
                    ...
                    <div className="grid sm:grid-cols-3 gap-3 text-xs">
                      {[
                        ['Lean FIRE','orange','Retiring on a highly frugal budget that covers only basic survival expenses.'],
                        ['Coast FIRE','blue','Having enough invested early so it will grow to Full FIRE by standard retirement age without further savings.'],
                        ['Fat FIRE','purple','Retiring with an abundant budget allowing for premium healthcare, travel, and luxury living.'],
                      ].map(([title, color, desc]) => (
                        <div key={title} className="p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800/80">
                          <p className={`font-bold text-xs text-${color}-600 dark:text-${color}-400`}>{title}</p>
        ...
        {pageTab === 'tax' && (() => {
          ...
          const result       = buildTaxResult({ fundType: taxFundType, gain, holdingMonths, taxSlab, sellDate: taxSellDate });
          const { taxType, taxAmount, fyLabel, rulesNote, steps, pillColor } = result;
          ...
          return (
            <div className="grid lg:grid-cols-[380px,1fr] gap-6">
              ...
              <div className="card p-6 space-y-5">
                <div>
                  <h2 className="font-bold text-slate-900 dark:text-white mb-1">🏛️ Tax P&L Calculator</h2>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Auto-detects correct tax rules from your sell date. Rules update when new budget comes.
                  </p>
                </div>

                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Fund Type</p>
                  <div className="flex gap-2">
                    {[['equity','📈 Equity / ELSS / Index'],['debt','🏦 Debt / Liquid']].map(([v,l]) => (
                      <button key={v} onClick={() => setTaxFundType(v)}
                ...
                <SIPSlider id="tax-buy-amount" label="Amount Invested (Buy Value)" value={taxBuyAmount} onChange={setTaxBuyAmount} min={1000} max={10000000} step={1000} prefix="₹" formatFn={formatINR} />
                <SIPSlider id="tax-sell-amount" label="Amount Received (Sell Value)" value={taxSellAmount} onChange={setTaxSellAmount} min={1000} max={10000000} step={1000} prefix="₹" formatFn={formatINR} />

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label htmlFor="tax-buy-date" className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Buy Date</label>
                    <input id="tax-buy-date" type="date" value={taxBuyDate} onChange={e => setTaxBuyDate(e.target.value)} className="input-base py-2 text-sm w-full" />
                  </div>
                  <div>
                    <label htmlFor="tax-sell-date" className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider block mb-1">Sell Date</label>
                    <input id="tax-sell-date" type="date" value={taxSellDate} onChange={e => setTaxSellDate(e.target.value)} className="input-base py-2 text-sm w-full" />
                  </div>
                </div>

                {!isEquity && (
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Your Income Tax Slab</p>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">Choose the slab matching your total annual income.</p>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl px-3 py-2">
                  ...
                  <div>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Rules Auto-Applied</p>
                    <p className="text-xs font-bold text-slate-800 dark:text-slate-200">{fyLabel}</p>
                    <p className="text-[10px] text-slate-400 leading-relaxed mt-0.5">{rulesNote}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {/* Gain / Loss hero */}
                <div className={`card p-6 border-2 ${gain >= 0 ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-950/40' : 'border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/40'}`}>
                  <div className="flex items-start justify-between flex-wrap gap-2">
                    <div>
                      <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Gain / Loss</p>
                      <p className={`text-4xl font-black tabular-nums ${gain >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                        {gain >= 0 ? '+' : ''}{formatINR(gain)}
                      </p>
                      <p className="text-xs text-slate-500 mt-1">
                        Held <strong className="text-slate-800 dark:text-slate-200">{holdingDays} days</strong> ({(holdingMonths/12).toFixed(1)} yrs)
                      </p>
                    </div>
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${pillColor}`}>{taxType}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="card p-4 text-center">
                    <p className="text-[10px] text-slate-500 mb-1">Tax Type</p>
                    <p className="font-bold text-xs text-slate-900 dark:text-white leading-snug">{taxType}</p>
                  </div>
                  <div className="card p-4 text-center bg-red-50 dark:bg-red-950/30 border-red-100 dark:border-red-900">
                    <p className="text-[10px] text-slate-500 mb-1">Tax Payable</p>
                    <p className="font-bold text-sm text-red-600 dark:text-red-400">{formatINR(Math.round(taxAmount))}</p>
                  </div>
                  <div className="card p-4 text-center bg-emerald-50 dark:bg-emerald-950/30 border-emerald-100 dark:border-emerald-900">
                    <p className="text-[10px] text-slate-500 mb-1">Net in Hand</p>
                    <p className="font-bold text-sm text-emerald-600 dark:text-emerald-400">{formatINR(netInHand)}</p>
                  </div>
                </div>

                {/* Step-by-step plain English explanation */}
                <div className="card p-4 space-y-3">
                  <p className="font-bold text-slate-900 dark:text-white text-sm">📖 Why this tax? (Simple explanation)</p>
                  {steps.map((step, i) => (
                    <div key={i} className="flex gap-3 items-start">
                      <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-base flex-shrink-0">{step.icon}</div>
                      <div className="flex-1">
                        <p className="text-xs font-bold text-slate-800 dark:text-slate-200 mb-0.5">{step.title}</p>
                ...
                <div className="card p-4 space-y-2 text-xs text-slate-600 dark:text-slate-400">
                  <p className="font-bold text-slate-800 dark:text-slate-200 text-sm">📋 Complete Tax Breakdown</p>
                  {[
                    ...
                    ['Net in Hand',      formatINR(taxSellAmount - Math.round(taxAmount * 1.04))],
                  ].map(([l, v]) => (
                    <div key={l} className="flex justify-between border-b border-slate-100 dark:border-slate-700 pb-1.5 last:border-0 last:font-bold last:text-slate-900 last:dark:text-white">
                ...
                <div className="card p-3 bg-amber-50 dark:bg-amber-950 border-amber-200 dark:border-amber-800 text-[10px] text-amber-700 dark:text-amber-300 leading-relaxed">
                  ⚠️ Indicative only. Consult a CA for exact tax liability. This tool auto-updates rules from the <code className="bg-amber-100 dark:bg-amber-900 px-1 rounded">TAX_RULES</code> config — to add new budget rates, update the config in the source file.
                </div>
              </div>
            </div>
          );
        ...
        {pageTab === 'date' && (
          <div className="space-y-6">
            <div className="card p-5">
              <h2 className="font-bold text-slate-900 dark:text-white mb-1">📅 SIP Date Optimizer</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">See which SIP date historically gave the best XIRR for your chosen fund.</p>
            </div>
            <div className="grid lg:grid-cols-[380px,1fr] gap-6">
              <div className="card p-6 space-y-5">
                <div className="relative">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block mb-2">Select Fund</label>
                  <input type="text" value={dateQuery} onChange={e => { setDateQuery(e.target.value); setDateSearchOpen(true); }}
                  ...
                  {dateSearchOpen && dateFundSearch.length > 0 && (
                    <div className="absolute z-30 top-full mt-1 left-0 right-0 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl max-h-52 overflow-y-auto">
                      {dateFundSearch.map(f => (
                        <button key={f.schemeCode} type="button" onClick={() => handleDateFundSelect(f)}
                          ...
                          <span className="font-medium text-slate-900 dark:text-white line-clamp-1">{f.schemeName}</span>
                          <span className="text-slate-500 ml-2">#{f.schemeCode}</span>
                        </button>
                      ))}
                    </div>
                  )}
                </div>
                <SIPSlider id="date-sip-amount" label="Monthly SIP Amount" value={dateSipAmount} onChange={setDateSipAmount} min={500} max={100000} step={500} prefix="₹" formatFn={formatINR} />
                <div>
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">SIP Duration</p>
                  <div className="flex gap-2 flex-wrap">
                    {[1,3,5,7,10].map(y => (
                      <button key={y} onClick={() => setDateSipYears(y)}
                ...
                {dateFundLoading && <div className="text-xs text-slate-500 flex items-center gap-2"><span className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin inline-block" />Loading NAV data…</div>}
                {selectedDateFund && !dateFundLoading && (
                  <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-3 text-xs">
                    <p className="font-bold text-slate-800 dark:text-slate-200 line-clamp-2">{selectedDateFund.schemeName}</p>
                    <p className="text-slate-500 mt-0.5">#{selectedDateFund.schemeCode}</p>
                  </div>
                )}
              </div>
              <div className="space-y-4">
                {!selectedDateFund && (
                  <div className="card p-12 text-center">
                    ...
                    <p className="font-semibold text-slate-700 dark:text-slate-300 mb-1">Search for a fund to get started</p>
                    <p className="text-xs text-slate-400">We'll simulate your SIP on each date 1–28 using actual NAV history.</p>
                  </div>
                )}
                {selectedDateFund && !dateFundLoading && dateOptimizerResults.length > 0 && (
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="card p-4 bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-800 text-center">
                        <p className="text-[10px] text-emerald-700 dark:text-emerald-300 font-bold mb-1">🏆 Best SIP Date</p>
                        <p className="text-3xl font-black text-emerald-600 dark:text-emerald-400">
                          {bestDateResult ? `${bestDateResult.date}th` : '—'}
                        </p>
                        <p className="text-xs text-emerald-700 dark:text-emerald-400 mt-1">
                          {bestDateResult?.xirr != null ? `Est. XIRR: ${bestDateResult.xirr.toFixed(2)}%` : ''}
                        </p>
                      </div>
                      <div className="card p-4 bg-red-50 dark:bg-red-950/40 border-red-200 dark:border-red-800 text-center">
                        <p className="text-[10px] text-red-700 dark:text-red-300 font-bold mb-1">📉 Worst SIP Date</p>
                        <p className="text-3xl font-black text-red-600 dark:text-red-400">
                          {worstDateResult ? `${worstDateResult.date}th` : '—'}
                        </p>
                        <p className="text-xs text-red-700 dark:text-red-400 mt-1">
                          {worstDateResult?.xirr != null ? `Est. XIRR: ${worstDateResult.xirr.toFixed(2)}%` : ''}
                        </p>
                      </div>
                    </div>
                    <div className="card p-4">
                      <p className="font-bold text-slate-900 dark:text-white text-sm mb-3">XIRR by SIP Date</p>
                      <div className="grid grid-cols-7 gap-1.5">
                        {dateOptimizerResults.map(r => {
                          const allXirr = dateOptimizerResults.map(x => x.xirr ?? 0);
                          const minX = Math.min(...allXirr), maxX = Math.max(...allXirr);
                          const pct = maxX > minX ? ((r.xirr ?? minX) - minX) / (maxX - minX) : 0.5;
                            ...
                            <div key={r.date} className="flex flex-col items-center gap-1">
                              <div className="w-full flex flex-col items-center justify-end h-14">
                                <div
                                  ...
                                  style={{ height: `${Math.max(15, pct * 100)}%` }}
                                  title={`${r.date}th: ${r.xirr?.toFixed(2)}%`}
                                />
                              </div>
                              <span className={`text-[9px] font-bold ${isB ? 'text-emerald-600' : isW ? 'text-red-500' : 'text-slate-500'}`}>{r.date}</span>
                            </div>
                          );
                        })}
                      </div>
                      <div className="flex gap-3 mt-3 text-[10px] text-slate-500">
                        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-emerald-500 inline-block"/>Best</span>
                        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-red-400 inline-block"/>Worst</span>
                        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-blue-400 inline-block"/>Other dates</span>
                      </div>
                    </div>
                    <div className="card p-3 bg-amber-50 dark:bg-amber-950 border-amber-200 dark:border-amber-800 text-[10px] text-amber-700 dark:text-amber-300">
                      ⚠️ Results are historical simulations. SIP date impact is typically small (&lt;0.5% XIRR difference). Choose a date convenient for your cash flow.
                    </div>
                  </>
                )}
              </div>
